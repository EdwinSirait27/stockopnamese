...
use App\Models\Mtokodetsoglose001;
...
{
    // Ambil parameter db dari request
    $db = $request->get('db', 'mysql_third'); // default ke mysql_first
    
    // Simpan pilihan DB ke session biar bisa dipakai di query lain
    session(['selected_db' => $db]);

    return view('pages.dashboard', compact('db'));
}
     ...
     public function store(Request $request)
{
    // Validasi data
    $request->validate([
        'db' => 'required|in:mysql_third,mysql_fourth,mysql_fifth',
        'location' => 'required|string',
    ]);

    $db = $request->db;          // database yang dipilih user
    $bo = $request->location;    // BO yang dipilih user

    // UPDATE semua data di mtoko_det_soglo → isi kolom BO dengan BO yang dipilih
    DB::connection($db)->table('mtoko_soglo')->update([
        'BO' => $bo
    ]);

    return redirect()->route('dashboard', ['db' => $db])
                     ->with('success', "Semua data di $db.mtoko soglo berhasil diupdate dengan BO = $bo");
}
