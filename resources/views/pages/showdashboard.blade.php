                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4>Mtoko Det Soglo</h4>
                                <div class="card-header-action">
                                    <a href="{{ url()->previous() }}" class="btn btn-secondary ml-2">
                                        <i class="fas fa-arrow-left"></i> Back
                                    </a>
    <a href="{{ route('scan.page') }}" class="btn btn-primary">
        <i class="fas fa-plus-circle"></i> Create Data
    </a>
                                ...
                                <div class="entries-info d-flex justify-content-between align-items-center mb-3">
                                    <div class="entries-select">
                                        <label for="entries-select">Show
                                            <select id="entries-select" class="form-select form-select-sm d-inline-block"
                                                style="width: auto;">
    ...
    <script>
        jQuery(document).ready(function($) {
            ...
            function loadData() {
                ...
                $.ajax({
                    url: '{{ route('mtokodetsoglo.mtokodetsoglo') }}',
                    type: 'GET',
                    data: {
            ...
            function renderCards(data) {
                ...
                data.forEach(function(item, index) {
                    ...
                    let BARA = highlightSearchTerm(item.BARA, currentSearch);
                    let NOURUT = highlightSearchTerm(item.NOURUT, currentSearch);
                    let FISIK = highlightSearchTerm(item.FISIK, currentSearch);
                    let BARCODE = highlightSearchTerm(item.BARCODE, currentSearch);
                    let ID = highlightSearchTerm(item.ID, currentSearch);
                    
                    html += `
                        <div class="store-card">
                            <div class="store-header">
                                <div class="store-code">#${rowNumber} - ${KDTOKO}</div>
                                <div class="store-name">${KDTOKO}</div>
                            </div>
                            <div class="store-info">
                                <div class="info-item">
                                    <div class="info-label">BARA</div>
                                    <div class="info-value">${BARA}</div>
                                </div>
                                <div class="info-item">
                                    <div class="info-label">NOURUT</div>
                                    <div class="info-value">${item.NOURUT || '-'}</div>
                                </div>
                                <div class="info-item">
                                    <div class="info-label">FISIK</div>
                                    <div class="info-value">${item.FISIK || '-'}</div>
                                </div>
                                <div class="info-item">
...
@section('main')
    <div class="main-content">
        <section class="section">
            <div class="section-header">
                <h1>KDTOKO : {{ $mtokosoglo->kdtoko }}</h1>
                <div class="section-header-breadcrumb">
                    <div class="breadcrumb-item active"><a href="/dashboard">Stock Opname</a></div>
                </div>
            </div>
            <div class="section-body">
                <div class="row">
                    <div class="col-12 col-md-12 col-lg-12">
                        <div class="card">
                            <div class="card-header">
                                <h4>Penginput : {{ $mtokosoglo->inpmasuk }}</h4>
                                <h4>Penghitung : {{ $mtokosoglo->personil }}</h4>
